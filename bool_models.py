...
class TransformerModel_XYZ(nn.Module):
    ...
    def forward(self, src: Tensor) -> Tensor:
        ...
        for i in range(src.size(0)):
            ...
            unpadded_x = x[:, unpadded_idx, :].permute(1, 0, 2)
...
class TransformerModel_XYZRGBD(nn.Module):
    ...
    def get_per_step_binaries(self, src: Tensor, input_images: Tensor, lengths: Tensor) -> Tensor:
        """
        Args:
            src: Tensor [seq_len, batch_size, 3]
            input_images: Tensor [seq_len, batch_size, width, height, 4]
            lengths: Tensor [batch_size, 2] where l[0] = t[-1] - t[0], l[1] = len(t)
