...
from bool_models import TransformerModel_XYZRGBD_Mask, generate_square_subsequent_mask

torch.multiprocessing.set_sharing_strategy('file_system')
writer = SummaryWriter("/home/zshureih/MCS/opics/output/logs/xyz_rgbdm_model_lr_1e-5")
...
class MCS_Sequence_Dataset(Dataset):
    ...
    def get_image_channels(self, n, bboxes, timesteps, scene_name, pad=100, root=dataset_dir):
        ...
        step_meta_path = os.path.join(root_folder, "Step_Output", f"step_{timesteps[0].item():06n}.json")
        step_meta = json.load(open(step_meta_path))
        
        # grab the color values of the target object (guaranteed one in scene for now)
        keys_list = list(step_meta["object_list"].keys())
        obj_mask_color = torch.Tensor(list(step_meta["object_list"][keys_list[n]]["segment_color"].values()))
        ...
        for t in timesteps.int():
            ...
            mask_path = os.path.join(root_folder, "Mask", f"{t.item():06n}.png")
            mask_im = F.to_tensor(Image.open(mask_path).convert('RGB')).permute(1, 2, 0)
            ...
            mask_window = rgb_transform(mask_im[int(min_y):int(max_y) + 1, int(min_x):int(max_x) + 1, :].permute(2,0,1))

            # remove anything from the mask that isn't strictly the focus object
            mask_window = mask_window.permute(1, 2, 0)
            mask_idx = torch.where(mask_window != (obj_mask_color / 255.))
            mask_window[mask_idx] = 0.0
            mask_window = depth_transform(mask_window.permute(2, 0, 1))

            rgb.append(rgb_window)
            depth.append(depth_window)
            mask.append(mask_window)
    ...
    def cook_tracks(self, object_tracks, track_lengths, scene_name, eval=False):
        ...
        for i in range(n):
            ...
            if eval:
                rgb, depth, mask = self.get_image_channels(i, bboxes, time, scene_name, root=validation_dir)
            else:
                rgb, depth, mask = self.get_image_channels(i, bboxes, time, scene_name, root=dataset_dir)
        ...
        mask_images = [img.unsqueeze(0) for img in mask_images[0]]
        mask_images = torch.cat(mask_images)

        return src, time, rgb_images, depth_images, mask_images
    ...
    def __getitem__(self, index):
        ...
        src, timesteps, rgb, depth, mask = self.cook_tracks(scene, track_lengths, scene_name[0], self.eval)

        input_image = torch.cat([rgb, depth, mask], axis=1)
...
if __name__ == "__main__":
    ...
    # img_enc_dim = 128
    img_enc_dim = 256**2
    model = TransformerModel_XYZRGBD_Mask(img_enc_dim + 3, 128, 8, 128, 2, 0.2).cuda()
    ...
    for epoch in range(max_epochs):
        ...
        if val_accuracies > best_val_acc:
            ...
            best_epoch = epoch
            # save model weights
            torch.save(best_model, f"./{epoch}_mask_when_hidden_xyz_rgbd_mask_model_all_5.pth")
    ...
    with open(f"{best_epoch}_mask_when_hidden_xyz_rgbd_mask_model_all_5.txt", "w+") as outfile:
